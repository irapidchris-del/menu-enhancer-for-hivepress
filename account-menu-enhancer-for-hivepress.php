 ...
 * @return object|false
 */
function amehp_inject_update( $transient ) {
	if ( ! is_object( $transient ) ) {
		return $transient;
	}

	$basename = plugin_basename( AMEHP_FILE );
	$release  = get_site_transient( AMEHP_UPDATE_CACHE_KEY );
	$version  = AMEHP_VERSION;
 ...
 * @return array<string, string>
 */
function amehp_add_bulk_check( $actions ) {
	if ( current_user_can( 'update_plugins' ) ) {
		$actions['hpx_check_updates'] = __( 'Check for updates', 'account-menu-enhancer-for-hivepress' );
	}
 ...
 * @return string
 */
function amehp_handle_bulk_check( $redirect, $action, $plugin_files ) {
	if ( 'hpx_check_updates' === $action && current_user_can( 'update_plugins' ) && in_array( plugin_basename( AMEHP_FILE ), (array) $plugin_files, true ) ) {
		amehp_get_latest_release( true );
	}
 ...
 * @return string
 */
function amehp_finish_bulk_check( $redirect, $action, $plugin_files ) {
 ...
 * @return void
 */
function amehp_show_bulk_check_notice() {
	...
	if ( $available ) {
		...
		$message = sprintf( _n( 'Checked %1$s plugin for updates: %2$s can be updated.', 'Checked %1$s plugins for updates: %2$s can be updated.', $checked, 'account-menu-enhancer-for-hivepress' ), number_format_i18n( $checked ), number_format_i18n( $available ) );
	} else {
		/* translators: %s: number of plugins checked. */
		$message = sprintf( _n( 'Checked %s plugin for updates: it is up to date.', 'Checked %s plugins for updates: all are up to date.', $checked, 'account-menu-enhancer-for-hivepress' ), number_format_i18n( $checked ) );
	}
 ...
 * @return void
 */
function amehp_print_plugins_screen_styles() {
...
add_filter( 'site_transient_update_plugins', 'amehp_inject_update' );
add_filter( 'handle_bulk_actions-plugins', 'amehp_handle_bulk_check', 10, 3 );
...
if ( empty( $GLOBALS['hpx_update_check_bulk'] ) ) { // phpcs:ignore WordPress.NamingConventions.PrefixAllGlobals.NonPrefixedVariableFound -- shared handshake between every copy of this updater; a plugin-specific prefix would defeat it.
	$GLOBALS['hpx_update_check_bulk'] = 'account-menu-enhancer-for-hivepress'; // phpcs:ignore WordPress.NamingConventions.PrefixAllGlobals.NonPrefixedVariableFound -- shared handshake between every copy of this updater; a plugin-specific prefix would defeat it.

	add_filter( 'bulk_actions-plugins', 'amehp_add_bulk_check' );
	add_filter( 'handle_bulk_actions-plugins', 'amehp_finish_bulk_check', 20, 3 );
	add_action( 'admin_notices', 'amehp_show_bulk_check_notice' );
	add_action( 'network_admin_notices', 'amehp_show_bulk_check_notice' );
	add_action( 'admin_print_styles-plugins.php', 'amehp_print_plugins_screen_styles' );
}
